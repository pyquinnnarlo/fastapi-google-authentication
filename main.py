...
async def google_register(request: Request):
    """
    Initiates the registration process by redirecting the user to the Google OAuth authorization page.

    Parameters:
        request (Request): The incoming request object containing information about the HTTP request.
    Returns:
        Response: A redirect response to the Google OAuth authorization page for user registration.
    Example:
        ...
        Google account for registration. Upon successful authentication, the user will be redirected back
        to the provided redirect URI with an authorization code.
    """
    redirect_uri = request.url_for('google_register_auth')
    return await oauth.google.authorize_redirect(request, redirect_uri)

@app.get('/google-register/auth')
async def google_register_auth(request: Request):
    """
    Handles the callback from the Google OAuth authorization page after user registration.
    This endpoint receives the authorization code from Google OAuth and exchanges it for an access token.
    Upon successful registration, the user's information is retrieved from Google and stored in the session.

    Parameters:
        request (Request): The incoming request object containing information about the HTTP request.
    Returns:
        RedirectResponse: A redirect response to the root URL ('/') after successful registration.
        HTMLResponse: An HTML response displaying the error message if registration fails.
    Example:
        After the user registers on the Google OAuth authorization page, Google redirects
...
async def auth(request: Request):
    ...
    # Retrieve user information from Google OAuth
    user = token.get('userinfo')
    if user:
        request.session['user'] = dict(user)
    return RedirectResponse(url='/')
